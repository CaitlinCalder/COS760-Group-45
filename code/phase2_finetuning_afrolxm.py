...
from transformers import (
    ...
    TrainingArguments,
    Trainer,
    get_cosine_schedule_with_warmup #added this for the scheduler
)
...
AFROXLMR_MODEL = "Davlan/afro-xlmr-base" #changed to base model to prevent CUDA OutOfMemoryError as large was giving issues
...
MAX_LENGTH   = 512 #as languages tend to run longer and need more tokens
BATCH_SIZE   = 8
EPOCHS       = 5
LEARNING_RATE = 2e-5
SEED         = 42
...
train_df, val_df = train_test_split(
    ...
    test_size=0.2,
    random_state=SEED,
    stratify=train_val_df[["label", "model"]]  # ← ensures each LLM is represented
)
...
# Human vs Machine Generated Text Section
...
#a lightweight custom callback that works with any Trainer
class OptunaPruningCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        if metrics is not None and "eval_macro_f1" in metrics:
            ...
            # Report the score to Optuna at the end of the epoch
            self.trial.report(current_value, step=int(state.epoch))

            # If the trial is performing badly, tell Optuna to prune it
...
def get_layerwise_optimizer(model, base_lr=2e-5, decay_factor=0.9, weight_decay=0.01):
    ...
    num_layers = model.config.num_hidden_layers   # 12 base / 24 large
    ...
    #transformer layers: decayed per layer
    ...
    #embeddings: lowest learining rate
...
#freezing bottom N layers this prevents overwriting multilingual representations in early layers.
...
def objective(trial):
    ...
    try:
      trainer.train()
      result = trainer.evaluate(siswati_ds) #evaluteusing the siswati set
...
train_loss, eval_loss, eval_f1, epochs_logged = [], [], [], []
...
#Phase 1 vs Phase 2 grouped bar chart
metrics_labels = ["Precision", "Recall", "Macro F1", "MCC"]
p1_vals = [phase1.get(k, float("nan"))
           for k in ["precision", "recall", "macro_f1", "mcc"]]
p2_vals = [phase2.get(k, float("nan"))
           for k in ["precision", "recall", "macro_f1", "mcc"]]
...
#Dataset split summary bar chart
split_names  = ["Train\n(Zu+Xh)", "Val\n(Zu+Xh)", "Test\n(Siswati)"]
split_human  = [
    (train_df["label"] == 0).sum(),
    (val_df["label"]   == 0).sum(),
    (siswati_df["label"] == 0).sum(),
]
split_machine = [
    (train_df["label"] == 1).sum(),
    (val_df["label"]   == 1).sum(),
    (siswati_df["label"] == 1).sum(),
]

x = np.arange(len(split_names))
fig, ax = plt.subplots(figsize=(7, 4))
ax.bar(x, split_human,  label="Human",   color="#4C72B0", edgecolor="white")
ax.bar(x, split_machine, bottom=split_human,
       label="Machine", color="#C44E52", edgecolor="white")
...
#Error Analysis Confidence vs Correctness
siswati_conf   = machine_conf          # P(Machine) for each Siswati sample
siswati_correct = (ft_preds == true_labels).astype(int)

fig, ax = plt.subplots(figsize=(8, 4))
colors = {0: "#C44E52", 1: "#55A868"}   # red = wrong, green = correct
labels_map = {0: "Incorrect", 1: "Correct"}

for outcome in [0, 1]:
    mask = siswati_correct == outcome
    ax.scatter(siswati_conf[mask],
               np.random.uniform(-0.3, 0.3, mask.sum()),   # jitter on y
...
#predict on all languages, not just Siswati
...
def compute_group_metrics(group_df):
    ...
    #required at least both classes present for macro metrics
    if len(set(y_true)) < 2:
        return None
...
plt.tight_layout()
fig.savefig(f"{FIG_DIR}/cross_llm_per_language.png", dpi=150, bbox_inches="tight")
plt.show()
...
ax.set_ylabel("Generating Model")
plt.tight_layout()
fig.savefig(f"{FIG_DIR}/heatmap_model_language.png", dpi=150, bbox_inches="tight")
plt.show()

#Spidar Chart Per Model
...
for idx, row in model_metrics_df.iterrows():
    values_r = [row[m] for m in radar_metrics]
    values_r += values_r[:1]
...
plt.tight_layout()
save(fig, "16_grouped_bar_model_language")
plt.show()

#Confidence Box Plots

# results_df must have prob_machine, model, label columns
...
ax.legend()
plt.tight_layout()
save(fig, "17_confidence_boxplot_by_model")
plt.show()

#Error Rate Heatmap
error_pivot = metrics_df.pivot(index="model", columns="language", values="accuracy")
error_pivot = 1 - error_pivot   # flip to error rate

fig, ax = plt.subplots(figsize=(8, 5))
sns.heatmap(
    error_pivot, annot=True, fmt=".3f", cmap="OrRd",
    linewidths=0.5, vmin=0, vmax=1, ax=ax,
    annot_kws={"size": 10}
)
...
ax.legend()
plt.tight_layout()
save(fig, "19_perclass_f1_by_model")
plt.show()

metrics_df.to_csv(f"{BASE_DIR}/metrics_by_model_language.csv", index=False)
model_metrics_df.to_csv(f"{BASE_DIR}/metrics_by_model.csv", index=False)
